...
app.get("/random", async (req, res) => {
  try {
    const count = await api_1.countDocuments();
    if (count === 0) return res.status(404).send("No jobs available");
    const jobGet = Math.floor(Math.random() * count);
    const givenJob = await api_1.findOne().skip(jobGet); // Using skip to fetch a random job
    res.json(givenJob);
  } catch (error) {
    res.status(500).send("Error fetching random job: " + error.message);
  }
});
